...
def _assessment(assessment_id: str, signal_id: str) -> RiskAssessment:
...
def test_approval_flow(tmp_path: Path) -> None:
    settings = get_settings().model_copy(
        update={
            ...
            "persistence_db_url": f"sqlite:///{tmp_path / 'approval_flow.db'}",
        }
    )
    ...
    assessment = _assessment("ras_approval_flow_001", "sig_approval_flow_001")
    ...
    persisted_pending = approvals_repo.get_approval(approval.approval_id)
    executed = asyncio.run(service.approve_and_execute(approval.approval_id))
    persisted_executed = approvals_repo.get_approval(approval.approval_id)

    reject_assessment = _assessment("ras_approval_flow_002", "sig_approval_flow_002")
    reject_service = ExecutionService(
        settings=settings,
        risk_service=FakeRiskService(reject_assessment),
        market_data_service=FakeMarketDataService(),
        approvals_repo=approvals_repo,
        trades_repo=trades_repo,
        positions_repo=positions_repo,
    ...
    reject_approval = asyncio.run(reject_service.create_approval_from_assessment(reject_assessment.assessment_id))
    rejected = asyncio.run(reject_service.reject(reject_approval.approval_id, reason="manual"))
    persisted_rejected = approvals_repo.get_approval(reject_approval.approval_id)
    ...
    assert trades_repo.list_trades("paper")
    assert rejected.status == "rejected"
    assert persisted_rejected is not None
    assert persisted_rejected.rejection_reason == "manual"
